...
const CheckoutPage = () => {
  ...
  // Fetch profile và auto điền form
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentMethod === 'cash') {
      ...
      try {
        ...
        navigate('/orders'); // Điều hướng sang trang danh sách đơn hàng
      } catch (error) {
        ...
        <div className="order-summary bg-gray-50 p-6 rounded-lg">
          <div className="space-y-4">
            ...
            <div className="flex space-x-4 mt-6">
              <input type="text" placeholder="Coupon Code" className="flex-1 px-4 py-2 border rounded" />
              <button className="bg-red-500 text-white px-6 py-2 rounded">Apply Coupon</button>
            </div>

            <button onClick={handleSubmit} className="w-full bg-red-500 text-white py-3 rounded mt-6">
              Place Order
            </button>
          </div>
        </div>
